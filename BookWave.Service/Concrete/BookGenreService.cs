...
{
    public async Task<ServiceResult<int>> CreateAsync(CreateBookGenreDto request)
    {
        var valueAsDto = mapper.Map<BookGenre>(request);
        await repository.AddAsync(valueAsDto);
        await unitOfWork.SaveChangesAsync();
        return ServiceResult<int>.SuccessAsCreated(valueAsDto.Id, $"api/bookgenres/{valueAsDto}");
    }
