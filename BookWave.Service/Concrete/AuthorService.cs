...
{
    public async Task<ServiceResult<int>> CreateAsync(CreateAuthorDto request)
    {
        var anyValue = await repository.Where(x => x.Name == request.Name).AnyAsync();

        if (anyValue)
            return ServiceResult<int>.Fail("An author with the same name already exists.", HttpStatusCode.BadRequest);

        var valueAsDto = mapper.Map<Author>(request);
        await repository.AddAsync(valueAsDto);
        await unitOfWork.SaveChangesAsync();
        return ServiceResult<int>.SuccessAsCreated(valueAsDto.Id, $"api/authors/{valueAsDto}");
    }
