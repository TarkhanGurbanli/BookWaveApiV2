...
public class GenresController(IGenreService service) : CustomBaseController
{
    [HttpGet]
    ...
    [HttpPost]
    public async Task<IActionResult> Create(CreateGenreDto request)
       => CreateActionResult(await service.CreateAsync(request));

    [HttpPut("{id:int}")]
    public async Task<IActionResult> Update(int id, UpdateGenreDto request)
